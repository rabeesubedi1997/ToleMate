...
class BookingController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        NotificationController::sendNotification(
            ...
            'booking',
            'New booking request',
            "{$user->name} wants to book: {$service->name}",
            [
        ...
        if ($vendorUser && $vendorUser->email) {
            try {
                Mail::to($vendorUser->email)->queue(new BookingMail(
                    ...
                    mailSubject: "New Booking Request – {$service->name}",
    ...
    public function update(Request $request, $id)
    {
        ...
        $vendorBiz   = $booking->service->vendor->business_name ?? $booking->service->vendor->user?->name ?? '';
        if ($oldStatus !== $booking->status) {
            ...
            $statusTitle = $statusTitles[$booking->status] ?? 'Booking updated';
            NotificationController::sendNotification(
                $recipientId,
                'booking',
                $statusTitle,
                "{$senderName}: {$booking->service->name}",
                [
                    'booking_id'   => $booking->id,
                    'status'       => $booking->status,
                    'service_name' => $booking->service->name,
                    'vendor_name'  => $vendorBiz,
                    'price'        => $booking->price,
                ]
            );
            ...
            if ($recipient && $recipient->email) {
                ...
                try {
                    Mail::to($recipient->email)->queue(new BookingMail(
                        ...
                        mailSubject: "Booking {$statusLabel} – {$booking->service->name}",
    ...
    public function createRequest(Request $request)
    {
        ...
        // Fan-out: notify all vendors with active services in the same category.
        // Uses sendNotification() (not bulk insert) so the SSE stream picks up each notification
        // and vendors see the InDrive-style popup instantly while they are online.
        $notifyCategory = $request->category_id
            ? \App\Models\Category::find($request->category_id)
            : null;
        $categoryName = $notifyCategory?->name ?? 'General';

        $vendorUserIds = \App\Models\Service::where('is_active', true)
            ...
            ->unique()
            ->filter()
            ->reject(fn($uid) => $uid === $user->id) // don't notify the requester if they are also a vendor
            ->values();
        ...
        foreach ($vendorUserIds as $uid) {
            NotificationController::sendNotification(
                ...
                "{$user->name} needs: \"{$bookingRequest->title}\"" . ($bookingRequest->budget ? " - Budget: Rs. " . number_format($bookingRequest->budget) : ''),
                [
                    ...
                    'text_preview'  => mb_substr($bookingRequest->text, 0, 120),
                ]
            );
        }
