...
const fmtWA = (p: string | null | undefined) => (p || '').replace(/\D/g, '');
...
const VendorPublicProfile: React.FC = () => {
  ...
  useEffect(() => {
    const fetchVendor = async () => {
      try {
        ...
        // Fetch availability
        try {
          const { data: ad } = await api.get(`/vendors/${id}/availability`);
          setAvailability(ad.availability || []);
        } catch { /* ignore */ }
        // Fetch portfolio
        try {
          const { data: pd } = await api.get(`/vendors/${id}/portfolio`);
          setPortfolio(pd.portfolio || []);
        } catch { /* ignore */ }
        // Fetch bundles
        try {
          const { data: bd } = await api.get(`/vendors/${id}/bundles`);
  ...
  useEffect(() => {
    ...
    const fetchReviews = async () => {
      try {
        const { data } = await api.get(`/vendors/${id}/reviews`, { params: { page: reviewsPage } });
        ...
        <div className="card p-6 md:p-8 mb-6">
          <div className="flex flex-col md:flex-row gap-6 items-start md:items-center">
            ...
            <div className="flex flex-col gap-2 flex-shrink-0 w-full md:w-auto">
              ...
              {vendor.whatsapp_enabled && (vendor.whatsapp_number || vendor.user?.phone) && (
                <>
                  <a
                    href={`https://wa.me/${fmtWA(vendor.whatsapp_number || vendor.user?.phone)}?text=${encodeURIComponent(`Hi ${vendor.business_name}, I found you on ToleMate and would like to enquire about your services.`)}`}
                    target="_blank"
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            {vendor.services.length === 0 ? (
              ...
              vendor.services.map(s => (
                <Link key={s.id} to={serviceUrl(s)} className="card p-5 flex justify-between items-start hover:shadow-md transition-shadow gap-4 group">
                  <div className="flex-1 min-w-0">
