...
import { ArrowLeft, Calendar, CheckCircle, Clock, XCircle, CreditCard, MessageCircle, Star, Download, RefreshCw, Banknote } from 'lucide-react';
...
const BookingDetail: React.FC = () => {
  ...
  const handleMarkCashPaid = async () => {
    ...
    setMarkingCash(true);
    try {
      const res = await fetch(`${API_BASE}/api/bookings/${booking.id}/mark-cash-paid`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` },
      });
      const data = await res.json();
      if (res.ok) {
        setBooking(prev => prev ? { ...prev, ...data.booking } : prev);
        toast('Cash payment marked as received!');
      } else {
        toast(data.message || 'Failed to mark cash', 'error');
      }
    } catch { toast('Error updating payment', 'error'); }
    finally { setMarkingCash(false); }
  };
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          {user?.role === 'vendor' &&
            ...
            <button
              ...
              {markingCash ? 'Saving…' : 'Mark cash received'}
            </button>
          )}

          {/* Customer: pay now (digital) or show COD notice */}
          {user?.role !== 'vendor' &&
            booking.payment_status !== 'paid' &&
            ['accepted', 'in_progress'].includes(booking.status) && (
            booking.payment_method === 'cod' ? (
              <div className="flex items-center gap-2 px-4 py-2.5 bg-yellow-50 border border-yellow-200 rounded-xl text-sm text-yellow-700 flex-1">
                <Banknote className="w-4 h-4 flex-shrink-0" />
                <span>Pay cash to vendor on arrival (COD)</span>
              </div>
            ) : (
              <Link to={`/checkout/${booking.id}`} className="btn-primary flex-1 justify-center flex items-center gap-2">
                <CreditCard className="w-4 h-4" /> {booking.payment_method ? 'Pay now' : 'Pay now'}
              </Link>
            )
          )}

          {['pending', 'accepted'].includes(booking.status) && !booking.reschedule_status && user?.role !== 'vendor' && (
          ...
          <Link
            ...
            {user?.role === 'vendor' ? 'Chat with customer' : 'Chat with vendor'}
          </Link>
          <button
