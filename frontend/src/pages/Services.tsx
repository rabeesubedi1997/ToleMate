...
const Services: React.FC = () => {
  ...
  // Re-fetch when geo coords arrive — only if Near Me is active
  const coordsRef = useRef(coords);
  const nearMeActiveRef = useRef(nearMeActive);
  useEffect(() => { nearMeActiveRef.current = nearMeActive; }, [nearMeActive]);
  useEffect(() => {
    coordsRef.current = coords;
    // Only re-fetch on coords update if the user explicitly activated Near Me
    if (coords && nearMeActiveRef.current) fetchServices(new URLSearchParams(location.search), 1, false);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [coords]);
  ...
  const fetchServices = async (params: URLSearchParams, pg = 1, append = false) => {
    ...
    try {
      ...
      // Only apply location filter when user explicitly enabled Near Me
      const c = nearMeActiveRef.current ? coordsRef.current : null;
      if (c) {
