                ...
                <div className="px-4 pt-4 pb-0 flex items-center justify-between gap-2">
                  ...
                  <button
                    onClick={() => {
                      ...
                      const header = 'Date,Service,Customer,Amount (Rs.)';
                      const rows = completed.map(b => [
                        new Date(b.scheduled_time || b.created_at).toLocaleDateString(),
                    ...
                    title="Export completed bookings as CSV"
                    className="flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg border border-gray-200 text-gray-600 hover:bg-gray-50 whitespace-nowrap flex-shrink-0"
                  >
                    ⬇ Export CSV
                  </button>
                </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {activeTab === 'requests' && bookingRequests.map(req => (
                      <tr key={req.id} className="hover:bg-gray-50">
                        <td className="p-4"><p className="text-sm font-medium text-gray-900">{req.customer.name}</p><p className="text-xs text-gray-400">{req.customer.phone}</p></td>
                        <td className="p-4 text-sm text-gray-600 max-w-xs truncate">"{req.text}"</td>
                        <td className="p-4"><span className="badge badge-warning">{req.status}</span></td>
                        <td className="p-4 text-right"><button onClick={() => navigate('/marketplace')} className="btn-primary text-xs py-1.5 px-3">Respond</button></td>
                      </tr>
                    ))}
                    {activeTab === 'bookings' && allBookings
                      .filter(b => bookingFilter === 'all' ? true : bookingFilter === 'active' ? ['accepted','in_progress'].includes(b.status) : b.status === bookingFilter)
                      .map(b => (
                    ...
                    {activeTab === 'bookings' && allBookings.filter(b => bookingFilter === 'all' ? true : bookingFilter === 'active' ? ['accepted','in_progress'].includes(b.status) : b.status === bookingFilter).length === 0 && (
            ...
            <div className="space-y-4">
              {vendorReviews.length === 0 && (
                <div className="card p-12 text-center">
                  ...
                  <p className="text-gray-500 text-sm">No reviews yet. Complete bookings to receive reviews.</p>
                </div>
              )}
              {vendorReviews.map((rev: any) => (
