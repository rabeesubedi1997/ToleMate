...
## Session 8 (Aug 4) — Live verification sweep + critical bug fixes (user-reported)
User reported: login "succeeds for 1ms then returns to login page" + "app icon doesn't look like ToleMate". Root causes found and fixed, then EVERY new flow verified live on the emulator (API-33, debug build, Metro, backend 10.0.2.2:8001).

### Bugs found & fixed
1. **CRITICAL login flash (mobile/src/api/client.ts)** — Session 5's request interceptor returned `sanitizeConfig(config)`, which replaced the real `Authorization` header with `Bearer [REDACTED]` on EVERY request. Login succeeded → first authed request 401'd → interceptor wiped storage + `onAuthExpired()` → back to login screen. Fix: interceptor returns the real config; `sanitizeConfig` removed. Introduced in Session 5 and never caught because no live test ran after it — **verified fixed live** (login stays, session restore works).
2. **Booking creation crash (backend/app/Mail/BookingMail.php)** — `public readonly string $subject` redeclares parent `Illuminate\Mail\Mailable::$subject` (untyped, non-readonly) → PHP FatalError → any booking creation failed after the booking mail was instantiated. Fixed: `public $subject` (no type). Verified: booking created OK via UI + API.
3. **Package not persisted on bookings (backend)** — `bookings` table had NO `package_id` column; BookingController validated it for price resolution but never saved it (web has the same bug — BookService.tsx sends package_id too). Fix: migration `add_package_id_to_bookings_table` (nullable FK, nullOnDelete), Booking model fillable + `package()` relation, BookingController::store persists it, index/show eager-load `package`. Verified: booking 19 → package_id=1, show returns package "Standard".
4. **Mobile package display** — MyBookings + VendorBookings detail modals now show the PACKAGE row when present.

### Live E2E verification (all PASS)
- **Login**: superadmin, customer (david), vendor (john) — all log in and STAY logged in (bug fixed); logout → login screen; force-stop + relaunch → session restored to Home (no flash)
- **Booking flow**: Marketplace → ServiceDetail (cover image renders 1080x746, packages section "Choose a package" + Book Standard, tags, rating, reviews) → BookingForm (14-day strip, slots appear after date pick, instant/quote, package chip + Rs 200 price) → booking created (server-side verified)
- **Payments**: vendor accepted booking (PUT /bookings/16) → MyBookings shows "Pay now" on accepted+pending → Checkout screen (booking summary, Rs 150 + Rs 100 platform fee = Rs 250, Card/Khalti) → mock card payment success, DB `payment_status=paid`, `transaction_id=TXN_...` persisted
- **Vendor**: dashboard analytics, Services list (add/edit modal, hide/delete dialogs), pricing tier created via UI (Standard, Rs 200, 3d — persisted, `GET /services/1` shows packages), **cover photo upload E2E** (adb-pushed JPEG → system photo picker → POST /services/{id}/image → "Uploaded", server `images[0]` = /storage/services/1_*.jpg), Bundles screen (create modal, 2 services selected, "Home Repair Combo" → Rs 1200 → listed)
- **Customer-side display**: ServiceDetail packages (Standard Rs 200, 3d delivery, Book Standard), vendor public profile "Service Bundles" (Home, Rs 1200, both service chips)
- **Notifications**: real notification delivered ("Booking Status Updated — accepted", generated by vendor accept), filter chips (All/Unread/Booking/Message/Payment/Review), "Read all", collapsible "Notification preferences" — 5 type toggles (persisted AsyncStorage) + SMS toggle → **`PUT /user/profile` verified server-side (`sms_notifications=1` for david)**
- **Admin**: overview dashboard with live stats (16 users/9 vendors/13 bookings), service re-approval via API (edit puts service back to pending)
- All checks: tsc 0, eslint 0, jest 1/1

### Deliverable
- Version bumped to 1.1 (versionCode 2); fresh release APK at repo root: **`ToleMate-v1.1.apk`** (v1.0 removed). Phone icon: launcher caches the old icon — **uninstall the app first, then install v1.1** (the green ToleMate icon is correctly in the APK — verified pixels #16a34a/#159143 at all mipmap sizes)
