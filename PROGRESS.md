...
## Session 7 (Aug 4) — Web ↔ mobile feature parity (part 2: images, pricing, bundles, prefs, payments)
- **Vendor service image upload** (`VendorServicesScreen`): "Upload cover" in edit modal → `launchImageLibrary` (photo, q0.8) → `validateFile` (jpg/jpeg/png/webp ≤4MB, matches backend `POST /services/{id}/image` field `image`) → multipart FormData; local preview via plain RN `Image` (`coverLocal`), server images via `AppImage`; upload requires an existing service (Alert "Save first")
- **Pricing tiers / packages** (`VendorServicesScreen`): full CRUD block in service edit modal (name/price/delivery days/description/features) via `POST/PUT/DELETE /services/{id}/packages[/{pkg}]`, max 3 tiers, list w/ edit + delete; tiers loaded from `GET /services/{id}` show response (packages)
- **ServiceDetail packages UI**: packages section between description and reviews — pkgCard with name/price/desc/features/delivery + Book button → `BookingForm` with `{id, packageId, packageName, packagePrice}`; BookingForm shows package chip in summary, sends `package_id`, uses package price for instant bookings
- **Vendor bundles**: new `VendorBundlesScreen` (list GET /vendor/bundles, create modal w/ name/desc/service multi-select chips from GET /services + bundle price + discount %, delete w/ confirm); menu entry (shopping-bag, teal) in VendorProfile Work Tools; public bundles section (bundleCard + service chips) on VendorPublicProfile via `GET /vendors/{id}/bundles`
- **Notification preferences** (`NotificationsScreen`): filter chips (all/unread/booking/message/payment/review/system → `unread_only`/`type` params) + collapsible "Notification preferences" panel — per-type toggles to AsyncStorage `tolemate_notif_prefs` (matches web localStorage key), SMS toggle to AsyncStorage `tolemate_sms_pref` **and** `PUT /user/profile {sms_notifications}` (server-persisted, matches web)
- **Customer payment flow**: new `CheckoutScreen` (`{bookingId}` route) — booking summary (service/vendor/package/scheduled time), price + Rs 100 platform fee, Card (mock `POST /payments/mock` w/ payment_method + last-4) vs Khalti (`Linking.openURL` web checkout `${WEB_URL}/checkout/{bookingId}`, dev 10.0.2.2:3000); MyBookings detail modal gets "Pay now" button when `accepted && payment_status === 'pending'`
- **Verified**: mobile tsc 0 errors, eslint 0 errors, jest 1/1 pass. Fixes: FONT_SIZE unused import (VendorBundlesScreen), upload route corrected to `/services/{id}/image` after backend route check, BookingForm summaryPrice formatting
- **Routes added**: `VendorBundles`, `Checkout: {bookingId: number}` in `MainStack` + `types.ts`; `BookingForm` params extended with `packageId?/packageName?/packagePrice?`
